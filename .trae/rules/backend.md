...
- **Services `(backend/fastapi/api/modules/*/service.py`)**: Encapsulate all core business logic and multi-step workflows. Manage the database session transaction lifecycle explicitly here—handling `commit()`, `refresh()`, and `rollback()`—while calling repositories strictly for query execution and data persistence.
- **Repositories (`backend/fastapi/api/modules/*/repository.py`)**: Encapsulate all database access methods using **SQLAlchemy 2.0** or **SQLModel**. Keep operations purely focused on query execution and statement building. Do NOT call commit(), refresh(), or rollback() at the repository level; transaction management and unit-of-work lifecycle must be handled strictly at the service level.
